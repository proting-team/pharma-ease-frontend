  ...
  <div class="w-full">
    <div class="mb-6">
      ...
      <p class="text-gray-500 text-sm mt-1">
        Kelola data pegawai dan hak akses sistem Apotek Lamtama
      </p>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
      ...
      <div
        v-if="meta"
        class="px-6 py-4 border-t border-gray-200 flex items-center justify-between bg-white"
      >
        <span class="text-sm text-gray-500"
          >Showing page {{ meta.currentPage }} of {{ meta.lastPage }}</span
        >
        <div class="flex items-center gap-1">
          <button
            :disabled="!meta.prev"
            @click="goToPage(meta.prev ?? null)"
            class="px-3 py-1 border border-gray-300 rounded text-gray-600 hover:bg-gray-50 transition-colors disabled:opacity-50"
          >
            &lt;
          </button>
          <button
            class="px-3 py-1 border border-gray-300 rounded bg-gray-100 text-gray-800 font-medium"
          >
            {{ meta.currentPage }}
          </button>
          <button
            :disabled="!meta.next"
            @click="goToPage(meta.next ?? null)"
            class="px-3 py-1 border border-gray-300 rounded text-gray-600 hover:bg-gray-50 transition-colors disabled:opacity-50"
          >
            &gt;
          </button>
  ...
  >
    ...
    >
      ...
      <div class="p-5 overflow-y-auto">
        <div
          v-if="error"
          class="mb-4 p-4 bg-red-50 border border-red-200 rounded-lg text-red-700 text-sm whitespace-pre-line"
        >
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                v-model="form.email"
                type="email"
                placeholder="e.g., user@lamtama.com"
                class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#11764B] focus:border-[#11764B]"
                required
              />
            </div>
            ...
            <div>
              ...
              <p class="text-xs text-amber-600 mt-1">
                Min. 4 karakter, minimal 2 huruf besar &amp; 3 angka (contoh: JhonDoe-123)
              </p>
            ...
            <div>
              ...
              >
                ...
                <option v-for="shift in shiftsList" :key="shift.value" :value="shift.value">
                  {{ shift.label }}
                </option>
              </select>
            </div>
...
} from '@/api-services/repositories/employeeApi'
  ...
  try {
    const payload = {
      ...
      dateOfBirth: form.value.dateOfBirth
        ? new Date(form.value.dateOfBirth).toISOString()
        : undefined,
